...
machine_mac() {
  ...
  digest="$(printf '%s:%s:%s\n' "$resource_namespace" "$vm_set_id" "$machine" |
...
grep -Fq "Project       $resource_namespace" "$status_out"
grep -Fq 'LDAP          pending' "$status_out"
grep -Fq 'Gerrit URL    pending-start' "$status_out"
grep -Fq 'Jenkins URL   pending-start' "$status_out"
grep -Fq '  *For host DNS, run simulation/vm/tools/configure-systemd-resolved.sh --help' "$status_out"
grep -Fq 'Target SSH' "$status_out"
grep -Fq 'Role                User          Host             State' "$status_out"
grep -Fq 'gerrit              ci-operator   pending-start    pending-start' "$status_out"
grep -Fq 'jenkins-controller  ci-operator   pending-start    pending-start' "$status_out"
grep -Fq 'jenkins-agent       ci-operator   pending-start    pending-start' "$status_out"
...
grep -Fq 'LDAP          pending' "$status_out"
grep -Fq 'Gerrit URL    http://gerrit.example.test:8080/' "$status_out"
grep -Fq 'Jenkins URL   http://jenkins-controller.example.test:8080/login' "$status_out"
grep -Fq '  *For host DNS, run simulation/vm/tools/configure-systemd-resolved.sh --help' "$status_out"

...
vm_config_load() {
  ...
  case "$VM_DEBUG_PRESERVE_FAILED_BAKE" in
    0|1) ;;
    *) die "VM_DEBUG_PRESERVE_FAILED_BAKE must be 0 or 1" ;;
  esac
  ...
  export VM_DOMAIN_DISK_SIZE VM_DEBUG_PRESERVE_FAILED_BAKE
}
...
VM_DEBUG_PRESERVE_FAILED_BAKE=$(shell_quote "$VM_DEBUG_PRESERVE_FAILED_BAKE")
...
HARNESS_VM_SET_TARGET_SSH_DIR=$(shell_quote "$HARNESS_VM_SET_TARGET_SSH_DIR")

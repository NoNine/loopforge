...
vm_write_harness_evidence() {
  ...
  local q_mode q_timestamp q_run q_set q_namespace q_checkpoint q_command q_status
...
vm_cmd_preflight() {
  local libvirt_summary vm_set_summary
  vm_config_load "$HARNESS_ENV_FILE"
  ...
  vm_cmd_preflight_readonly_checks >/dev/null || {
    print_command_failure preflight "" "failed reason=libvirt-or-vm-set-preflight"
    return 1
  }
...
vm_cmd_init_run() {
  ...
  simulation_with_set_lock exclusive "$HARNESS_SET_LOCK" "$HARNESS_SET_ID" \
    vm_config_init_run_locked || return $?
  ...
  print_command_summary init-run "" "ok set-id=$HARNESS_SET_ID run-id=$HARNESS_RUN_ID"
}
...
vm_cmd_audit_state() {
  ...
  vm_libvirt_status_readonly >/dev/null
  vm_ssh_status_readonly >/dev/null
...
vm_workflow_downstream_steps() {
  vm_workflow_step create vm_command_with_lock exclusive vm_cmd_create || return $?
  vm_workflow_step start vm_command_with_lock exclusive vm_cmd_start || return $?
  vm_workflow_step status vm_command_with_lock shared vm_cmd_status || return $?
  vm_workflow_step prepare-artifacts vm_command_with_lock exclusive vm_cmd_prepare_artifacts "" || return $?
  vm_workflow_step stage-artifacts vm_command_with_lock exclusive vm_cmd_stage_artifacts "" || return $?
  vm_workflow_step configure-role vm_command_with_lock exclusive vm_cmd_configure_role "" || return $?
  vm_workflow_step validate-role vm_command_with_lock exclusive vm_cmd_validate_role "" || return $?
  vm_workflow_step configure-integration vm_command_with_lock exclusive vm_cmd_configure_integration || return $?
  vm_workflow_step validate-integration vm_command_with_lock exclusive vm_cmd_validate_integration || return $?
  vm_workflow_step prove-integration vm_command_with_lock exclusive vm_cmd_prove_integration || return $?
}
...
vm_cmd_run() {
  if vm_config_runtime_valid; then
    vm_config_load_runtime
    printf 'run: mode=resume run-id=%s set-id=%s\n' "$HARNESS_RUN_ID" "$HARNESS_SET_ID"
  else
    vm_config_load "$HARNESS_ENV_FILE"
    printf 'run: mode=fresh run-id=%s set-id=%s\n' "$HARNESS_RUN_ID" "$HARNESS_SET_ID"
    vm_workflow_step preflight vm_command_with_lock shared vm_cmd_preflight || return $?
    vm_workflow_step init-run vm_cmd_init_run || return $?
  fi
  vm_workflow_downstream_steps || return $?
  print_command_summary run "" "ok run-id=$HARNESS_RUN_ID set-id=$HARNESS_SET_ID"
}
...
vm_cmd_create() {
  ...
  print_command_summary create "" "ok set-id=$HARNESS_SET_ID baseline-prereqs=ready baseline-snapshot=ready"
}

vm_cmd_start() {
  local evidence log
  vm_config_load_runtime
  log="$(vm_path_bounded_log start)"
  vm_cmd_run_logged "$log" vm_cmd_start_steps || {
    evidence="$(vm_write_harness_evidence start fail "simulate.sh start" "$log" "VM-set startup or target OS SSH readiness failed")"
    print_command_failure start "" "failed reason=vm-set-start" "$log" "$evidence"
    return 1
  }
...
vm_cmd_stop() {
  local evidence log
  vm_config_load_runtime
  log="$(vm_path_bounded_log stop)"
  vm_cmd_run_logged "$log" vm_cmd_stop_steps || {
    evidence="$(vm_write_harness_evidence stop fail "simulate.sh stop" "$log" "VM-set graceful shutdown failed")"
    print_command_failure stop "" "failed reason=vm-set-stop" "$log" "$evidence"
    return 1
  }
  evidence="$(vm_write_harness_evidence stop pass "simulate.sh stop" "$log" "VM set shut down while retaining disks and generated state")"
  print_command_summary stop "" "ok set-id=$HARNESS_SET_ID"
}
...
vm_cmd_clean() {
  ...
  vm_cmd_run_logged "$log" vm_cmd_clean_steps || {
    ...
    evidence="$(vm_write_harness_evidence clean fail "simulate.sh clean" "$log" "VM generated runtime cleanup failed or VM set was not stopped")"
  ...
  print_command_summary clean "" "ok set-id=$HARNESS_SET_ID generated-state=cleaned"
}
...
vm_cmd_restore_baseline() {
  ...
  vm_cmd_run_logged "$log" vm_cmd_restore_baseline_steps || {
    ...
    evidence="$(vm_write_harness_evidence restore-baseline fail "simulate.sh restore-baseline" "$log" "Baseline restore failed ownership, snapshot validation, or stopped-state validation")"
  ...
  print_command_summary restore-baseline "" "ok set-id=$HARNESS_SET_ID baseline=restored"
}
...
vm_cmd_destroy() {
  ...
  detail="ok set-id=$HARNESS_SET_ID removed"

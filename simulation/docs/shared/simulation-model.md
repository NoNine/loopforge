...
predecessors, generated path custody, input publication, or recovery sequences.
...
## Generated Storage Boundary

Generated runtime output is not committed. Both backends separate reusable
simulation-set storage from immutable run storage, and the generated roots are
fixed rather than operator-selected. The canonical roots, common child paths,
content custody, sensitivity, and cleanup classes are owned by
`simulation/docs/shared/generated-state-layout.md`.

This public model uses the terms set root and run root to explain command
behavior. It does not define their directory inventory. Backend guides may
name paths needed for operator commands and describe backend-only realization
deltas, but they do not redefine shared generated storage.

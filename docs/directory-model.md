...
| `/var/lib/loopforge/staging/gerrit-artifacts-bundle/gerrit/` | Gerrit target | Gerrit role helper | Operator account and group; default example `ci-operator:ci-operator` | Readable by Gerrit helper | Gerrit WAR, templates, manifests, checksums | Must not include integration keys, secrets, or external Gerrit plugins | Gerrit install consumes this path only |
